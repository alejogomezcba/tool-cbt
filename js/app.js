...
function valoresItem() {
  ...
  document.getElementById("url").href =
    InternalApi + "/items/" + i + "?caller.scopes=admin";
}
...
let userResult = async (search) => {
  ...
  const userfound = await fetch(
    `${InternalApi}/cbt/users/${search}?caller.scopes=admin`
  );
  ...
  const infoSellerFound = await fetch(
    `${InternalApi}/users/${userParentId}?caller.id=${userParentId}&caller.scopes=admin`
  );
  ...
  const emailBounce = await fetch(
    `${InternalApi}/internal/email_addresses/${email}`
  );
  const emailBouncejson = await emailBounce.json();
  var emailBounceStatus = emailBouncejson.bouncing_mails.bouncing_status;

  const sellerShipping = await fetch(
    `${InternalApi}/cbt/shipping_preferences/user/${search}?caller.scopes=admin`
  );
...
function cleanSearch(){
    ...
    var bannerItem = document.getElementById('banner_item');
    var buttonClear = document.createElement("button");
    
    bannerItem.appendChild(buttonClear)

    buttonClear.innerHTML = "Limpiar búsqueda";
    
    var contSearchItem = document.getElementById("contenedor_searchitem");

    buttonClear.addEventListener("click", function(event) {

        var divItem = document.getElementById("result_item");
        var divItemVar = document.getElementById("result_variation");
        var divItemLinks = document.getElementById("links");
        var divItemImg = document.getElementById("images");

    contSearchItem.removeChild(divItem);
    contSearchItem.removeChild(divItemVar);
    contSearchItem.removeChild(divItemLinks);
    contSearchItem.removeChild(divItemImg);
    bannerItem.removeChild(buttonClear);
    });
...
let itemResult = async (search) => {
  ...
  divItem.id = "result_item";
  let divItemVar = document.createElement("div");
  divItemVar.id = "result_variation";
  let divItemLinks = document.createElement("div");
  divItemLinks.id = "links";
  let divItemImg = document.createElement("div");
  divItemImg.id = "images";
  divItemImg.classList.add("divItemImg");

  contSearchItem.appendChild(divItem);
  contSearchItem.appendChild(divItemVar);
  contSearchItem.appendChild(divItemLinks);
  contSearchItem.appendChild(divItemImg);
  ...
  const catfound = await fetch(
    `${InternalApi}/categories/${categoria}/shipping_preferences`
  );
  ...
  const parentItemPic = await fetch(
    `${InternalApi}/items/${cbtItemParent}?caller.scopes=admin`
  );
  ...
  const itemResFBM = await fetch(
    `${InternalApi}/fulfillment/restrictions/${search}`
  );
  ...
  const itemDimen = await fetch(
    `${InternalApi}/items/${search}/dimensions?caller.scopes=admin`
  );
